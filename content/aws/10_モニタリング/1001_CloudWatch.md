...
**CloudWatchメトリクス = 「ある数値が、時間とともにどう動いたか」の時系列データ**です。

例：
- EC2の **CPU使用率** が 10:00→30%、10:05→45%、10:10→80%…
- ELBの **リクエスト数**、RDSの **接続数**、Lambdaの **エラー回数**…

> [!tip] 見分けのコツ
> **「グラフの折れ線にできるもの = メトリクス」** です。
...
#### メトリクス vs ログ（ここで混乱しがち）

| 項目 | メトリクス | ログ |
|---|---|---|
| **中身** | 数値 | テキスト記録 |
| **例** | CPU 80%、失敗が3回 | "user_A が 10:03 にログイン失敗" |
| **特徴** | 数えられる・グラフにできる | 1件ずつの出来事の文章 |
| **向き** | トレンド分析・アラーム | 詳細な根因調査 |

#### メトリクスフィルター：ログから数値を作る

> [!info] メトリクスフィルタ
> **CloudWatch Logs に溜まったテキストログを走査して、「'login failed' という行が何回出たか」を数え、新しいメトリクス（数値）に変換する機能。**
>
> これで「ログイン失敗**回数**」というメトリクスができ、アラームで「3回超えたら通知」が可能になる。
> 
> **SAA第2問のDがまさにこの機能です。**
...
#### CloudWatch vs AWS Config（メトリクスの視点）

「IAMの設定が変更された」という情報は、**数値ではなく"状態の変化（イベント）"** です。

- 「設定変更」を表す**数値メーターが存在しない**ので、CloudWatchメトリクス監視では拾えない。
- 「構成が変わった」を検知するのは **AWS Config**（設備点検簿）の仕事。

**役割の早見表：**

| 必要なこと | 使うサービス | 理由 |
|---|---|---|
| **○○が△回を超えたら通知** | **CloudWatch + メトリクス** | 数値の監視 |
| **テキストログを検索・分析** | **CloudWatch Logs** | テキストの記録 |
| **誰が何をしたか（操作の証跡）** | **CloudTrail** | API監査 |
| **設定が変わったら検知** | **AWS Config** | 構成管理 |

> [!tip] 第2問⇄第6問を取り違えない方法
> 問題で「**○○が△△な状態になったら**」と言われたら Config、「**○○が△回を超えたら**」と言われたら CloudWatch。
